...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // vault 페이지 전용 코드
    if (currentPath.endsWith('vault.html')) {
        const vaultContent = document.querySelector('.vault-content');
        const popupContainer = document.getElementById('vault-popup-container');
        ...
        // vault 페이지 전용 이벤트 리스너들
        if (submitButton && passwordInput) {
            submitButton.addEventListener('click', () => {
                checkVaultPassword(passwordInput.value);
            });
    ...
    // About 메뉴 이벤트 (모든 페이지에서 필요)
    const aboutLink = document.querySelector('.header.a a');
    const aboutMenu = document.querySelector('.about_menu');
    if (aboutLink && aboutMenu) {
